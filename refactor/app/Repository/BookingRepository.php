    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            $validator = $this->validate($data,[
                ...
                'customer_phone_type' => [Rule::requiredIf(function()use($data){
                    if ($data['immediate'] == 'no'){

                        return !isset($data['customer_phone_type']) && !isset($data['customer_physical_type']);
            ...
            if ($data['immediate'] == 'yes') {
                $due_carbon = Carbon::now()->addMinute($data['immediatetime']);
            ...
            if ($due_carbon->isPast()) {
                ...
                $validator->after(function($validator){
                    $validator->errors()->add('due','Can\'t create booking in past');
                });
            ...
            isset($data['customer_phone_type']) ? $data['customer_phone_type'] = 'yes' : $data['customer_phone_type'] = 'no';
            isset($data['customer_physical_type']) ? $data['customer_physical_type'] = 'yes' : $data['customer_physical_type'] = 'no';
            ...
            if (in_array('normal', $data['job_for'])) {
                ...
                //spelling error in health below
            ...
            $data['job_for'] = $this->createdJobData($job);
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    private function jobForData($job){
    ...
    public function jobToData($job)
    {
        ...
        $data['job_for']=$this->jobForData($job);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    private function changeStatus($job, $data, $changedTranslator)
    {
        ...
        return ['statusChanged' => $statusChanged, 'log_data' => ['old_status' => $old_status]];
    }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    // return a query builder and build on it through filters
    // do not leak Requests in repositories
